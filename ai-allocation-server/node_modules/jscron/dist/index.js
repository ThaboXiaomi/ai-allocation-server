...
class jscron {
     ...
     */
    static parse(cron, startTimeUnix, endTimeUnix) {
        ...
        let zDay = null;
        const daysSinceEpoch = (date) => Math.floor(date.getTime() / day);
        const lastDay = daysSinceEpoch(endTimeDate);
        const getNextDay = (refDate) => {
            if (!refDate) {
        ...
        while (daysSinceEpoch(zDay = getNextDay(zDay)) <= lastDay) {
            if (!days.includes(zDay.getUTCDate()))
                continue;
            if (!months.includes(zDay.getUTCMonth() + 1))
                continue;
            if (!dows.includes(zDay.getUTCDay()))
                continue;
            ...
            let zDayHours = hours;
            if (isStartDay)
                zDayHours = zDayHours.filter((h) => h >= startTimeDate.getUTCHours());
            if (isEndDay)
                zDayHours = zDayHours.filter((h) => h <= endTimeDate.getUTCHours());
